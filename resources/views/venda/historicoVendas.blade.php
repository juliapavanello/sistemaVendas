...
@section('content')
        ...
        <div class="lista">
            ...
            <div class="items-lista {{ $i=1 }}">
                ...
                <div onclick="mostrarVenda({{ $item->id }})" class="item-lista pg-{{ ceil(($i++)/$qtdPorPg) }}" @if($item->id > $vendas[$qtdPorPg-1]->id) style="display: none" @endif>
                    ...
                    <div class="campo" style="width: 33%; text-align: start;">
                        @php
                            $caixasFiltrados = array_filter($caixas->toArray(), function($caixa) use ($item) {
                                return $caixa['venda_id'] == $item->id;
                            });
                            $primeiroCaixa = reset($caixasFiltrados);
                        @endphp

                        <p>R$ {{ $primeiroCaixa['dinheiro'] ?? '0,00' }}</p>
                    </div>
                    <div class="campo" style="width: 12%">
                    ...
                    <div class="campo" style="width: 12.5%">
                        <p>{{ 
                        count(array_filter($itensVenda->toArray(),function($itemVendido) use ($item) {
                            return $itemVendido['venda_id'] == $item->id;
                        })) }}</p>
                    </div>
                    ...
                    <div class="campo"
                        ...
                        <!--icones deletar e excluir-->
                        <svg width="20" height="20" viewBox="0 0 20 20" onclick="window.location='{{ route('vendas.edit', $item->id) }}'" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <form action="{{ route('vendas.destroy', $item->id) }}" method="post">
                            @csrf
                            @method("DELETE")
                            <label for="{{ $item->id }}">
    ...
    <script>
        function mostrarVenda(id) {
            document.getElementById("venda-vazia").classList.add("oculto");
            Array.from(document.querySelectorAll(".nota-fiscal")).forEach(nota => {
                nota.classList.add("oculto");
            });
            document.getElementById("nota-"+id).classList.remove("oculto");
        }
        ...
        function proximo() {
            if (botoesPg[(Math.ceil(pgAtual / 5)) * 5]) {
                if(document.getElementById("goStart")) document.getElementById("goStart").style.display = "flex"
                ...
                for (let i = 0; i < 5; i++) {
                    ...
                    try {
                        ...
                        if(document.getElementById("goEnd")) document.getElementById("goEnd").style.display = "none"
                    }
                }
        ...
        function anterior() {
            if (botoesPg[(Math.ceil(pgAtual / 5) - 1) * 5 - 1]) {
                if(document.getElementById("goEnd")) document.getElementById("goEnd").style.display = "flex"
                ...
                if (secao - 1 == 0 && document.getElementById("goStart")) document.getElementById("goStart").style.display = "none"
        ...
        function pesquisar(pesquisa) {
            ...
            let resultado = itens.filter(item => item.querySelectorAll('p')[0].textContent.includes(pesquisa));
